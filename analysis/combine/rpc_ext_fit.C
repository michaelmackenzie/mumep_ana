...
int rpc_ext_fit(TString process = "mumem", int selection = 20, TString tag = "", TString pdf_type = "auto") {
  ...
  // Create the PDF
  RooAbsPdf* model_pdf = get_rpc_ext_model(obs, process, selection, false).pdf_;
  RooAbsPdf* pdf = choose_pdf_model(pdf_type,
                                    hist_pdfs_,
                                    obs,
                                    data_hist,
                                    model_pdf,
                                    Form("%s_%i_rpc_ext_pdf", process.Data(), selection),
                                    "RPC (external) background",
                                    h);
