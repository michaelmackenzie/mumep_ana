...
int rpc_ext_fit(TString process = "mumem", int selection = 20, TString tag = "") {
  if(use_evtana_) set_evtana_defaults();
  init_physics(tag); //initialize normalization info
  const char* figdir = Form("figures/rpc_ext%s", (tag == "") ? "" : ("_"+tag).Data());
  gSystem->Exec(Form("[ ! -d %s ] && mkdir -p %s", figdir, figdir));

  //----------------------------------------------
  // Get the input data

  TH1* h = get_background_hist("rpc_ext", selection, "rpc_fit");
  ...
  const float scale = get_dataset_info("rpc_ext").norm(npot_);
  ...
  RooDataHist data_hist("rpc_ext_data_hist", "RPC (external) input", obs, h);

  // Create the PDF
  RooAbsPdf* pdf = get_rpc_ext_model(obs, process, selection, false).pdf_;

  //----------------------------------------------
  // Perform the fit

  const TString name = Form("%s_%i_rpc_ext", process.Data(), selection);
  enforce_uniform_if_sparse(h, obs, pdf, Form("%s_pdf", name.Data()), "RPC (external) background");
  ...
  RooRealVar norm(Form("%s_%i_rpc_ext_norm", process.Data(), selection), "RPC (external) norm", n_rpc);
  ...
  if(save_fit_plot_pair(c, frame, Form("%s/rpc_ext_fit_%i", figdir, selection))) return 10;

  //----------------------------------------------
  // Save the results

  return save_fit_workspace(process, selection, tag, "rpc_ext", "RPC (external)", pdf, obs, norm, hist_pdfs_);
}
