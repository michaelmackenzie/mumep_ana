...
bool use_poly_family_       = false; //polynomial
...
std::pair<int,double> add_exponentials(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
...
std::pair<int,double> add_powerlaws(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
  const int max_order = 3;
  double min_chi = 1.e10;
  int min_index = -1;
  double prev_chi = -1.;
...
std::pair<int,double> add_laurents(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
...
std::pair<int,double> add_inv_poly(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
...
std::pair<int,double> add_chebychevs(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
  const int max_order = 3;
  double min_chi = 1.e10;
  int min_index = -1;
  double prev_chi = -1.;
...
std::pair<int,double> add_bernsteins(RooDataHist& data, RooRealVar& obs, RooArgList& list, bool useSideBands, TString name, int verbose) {
  ...
  for(int order = 1; order <= max_order; ++order) {
    ...
    else if(use_fast_bernstein_) basePdf = create_fast_bernstein   (obs, order, name);
...
RooMultiPdf* create_envelope(RooRealVar& obs, RooCategory& cat, RooDataHist& data, bool useSideBands, TString name, const int verbose = 0) {
  RooArgList pdf_list;
  ...
  if(use_poly_family_) {
    result = add_bernsteins(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
    result = add_chebychevs(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
  }
  if(use_exp_family_) {
    result = add_exponentials(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
  }
  if(use_power_family_) {
    result = add_powerlaws(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
  }
  if(use_laurent_family_) {
    result = add_laurents(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
  }
  if(use_inv_poly_family_) {
    result = add_inv_poly(data, obs, pdf_list, useSideBands, name, verbose);
    if(result.second < chi_min) { chi_min = result.second; best_index = result.first; }
  }
